...
# fabrications. Furi genuinely CAN search; the sentence is true. It is also
...
_DEAD_END_OFFER_RE = re.compile(
    ...
    # ⚠️ THE MAGIC-WORD DEMAND ITSELF (2026-08-06). Live: "furi open fomi
    # folder" — the classifier returned an EMPTY reply, routing fell open to
    # chat, and the user was told "Say it as one direct instruction, e.g. 'open
    # the fomi folder'". The user's report is the whole argument: nobody can be
    # expected to remember a phrase that unlocks a capability Furi just said
    # it has.
    #
    # This is not the model misbehaving — four CAPABILITIES rules below
    # (unrouted action, sign-in, own-action, promise-nothing) each END by
    # telling it to ask for a rephrase, and it obeyed. Only the WEB rule had a
    # backstop, so files, folders, mail, calendar, the browser and the audit log
    # all dead-ended. The branches above are the vocabulary of ONE capability;
    # this one is anchored on the DEMAND, which every rule words the same way,
    # so it covers all of them at once and any future rule for free.
    #
    # High precision by construction: this phrasing is near-verbatim from those
    # rules and cannot occur in a real answer. A demand to re-say something is
    # never a legitimate reply to a request Furi can carry out itself.
...
#
# Deliberately says "handle that" rather than "look that up" (2026-08-06): the
# backstop now covers every capability, and a failed folder-open reported as a
# failed web search would be a small lie of exactly the kind this file spends
# most of its length preventing. It also must not end by asking for a rephrase
# — that is the defect, not the remedy.
_DEAD_END_FALLBACK = (
...
# ⚠️ THIS TEXT USED TO END IN A MAGIC-WORD DEMAND — "To actually do this, say
# it as a direct instruction, e.g. …" — and that was the SECOND surface of the
# 2026-08-06 complaint, in text we author deterministically rather than text a
# model improvised. It also collided with `_DEAD_END_OFFER_RE` above: harmless
# today (the guard only ever scans model deltas, never our own appended text)
# but a loaded gun for anyone who later widens the guard to the whole response,
# the way the impersonation guard already does. Both problems die with the
# phrase, and `test_no_deterministic_text_trips_the_dead_end_guard` keeps it
# dead. Asking the user to guess a phrase is never the remedy here: a re-ask
# now meets a retried classifier and a catalog that knows the capability.
_IMPERSONATION_CORRECTION = (
    "\n\n⚠️ **Correction from the Furi system:** the text above imitated a "
...
def _build_system_prompt(
    ...
    """Build the Furi OS system prompt, with optional memory context block.
    ...
    base = f"""You are Furi, a personal AI operating system — a digital extension of the user, not a generic assistant.
...
- Never refer to yourself as an AI, assistant, or chatbot. You are Furi.
...
- Action requests are detected and routed to that system BEFORE the message reaches you. If a request to act still reaches you here, it was not recognized as a task: do NOT pretend you did it and do NOT deny you can — say in ONE short sentence that you can do it, then ask them to say it as a direct instruction (e.g. "list the files in <folder>", "open my downloads folder", "email Jamil about dinner", "search the web for X"). Never explain the routing, the tool system, or why the request did not reach it: that is internal plumbing, it means nothing to the user, and it is not their problem to work around.
- CREDENTIAL SAFETY: NEVER ask the user for a password, PIN, passphrase, personal access token, 2FA code, or any other credential — and never accept one if it is offered. You cannot and must not collect credentials. Signing in to a website is a browser action: Furi opens the site's OWN sign-in page in a window, the user signs in there themselves, and Furi continues the task afterwards — Furi never sees or handles the password. So a request like "sign in to github and open my oldest repo" or "log into my account and download the invoice" is something you CAN do (via that browser sign-in hand-off); if it reaches you here, do NOT ask for a username/password and do NOT claim you signed in — tell the user you can do it and ask them to say it as one direct instruction (e.g. "sign in to github and open my oldest repo").
...
- OWN-ACTION HONESTY: Furi's own action history lives in an audit log this conversation cannot see. Never assert or deny from memory what Furi itself created, deleted, moved, renamed, sent, or ran ("the folder you created", "did you delete X?") — memory holds the user's life, not Furi's actions, and a name found there may be stale or wrong. Say you need to check the action record and ask the user to say it as a direct question (e.g. "tell me what you created today"), which routes to that record.
...
async def chat_stream(
    ...
    # Per-stage latency observability (2026-07-13, "Furi feels slow"): one
    ...
    async def event_generator():
        ...
        try:
            async for delta in provider.stream_chat(messages):
                ...
                # at all: Furi appearing to ask permission and then acting
        ...
        if dead_end and last_user_msg:
            # The model has admitted this turn needs a capability chat does not
            # have — a web lookup, or (since 2026-08-06) any action it just
            # asked the user to re-say. Take it at its word and DO the thing,
            # rather than handing the user a phrase to guess.
            logger.warning(
                "Chat LLM dead-ended on a capability it cannot run from here — "
                "routing missed this turn; rescuing it through the planner "
                "(session {})",
                session_id,
            )
            from app.api.task_router import rescue_unrouted_turn
            try:
                async for sse in rescue_unrouted_turn(
                ...
                logger.error(f"Dead-end rescue failed: {e}")
                await routing_trace.note_stream_outcome(
