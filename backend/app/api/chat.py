...
_IMPERSONATION_CORRECTION = (
    ...
    "system message but was generated by the conversation model — no task "
    "ran, no reminder was created, and nothing on your computer was touched. "
    "To actually do this, say it as a direct instruction, e.g. "
...
CAPABILITIES:
- You CAN act on the user's computer through a separate tool system: search/read/list files and folders, create/move/rename/delete files, run terminal commands and scripts, set reminders, and run long tasks in the background. Never claim you lack file-system or computer access.
- Action requests are detected and routed to that system BEFORE the message reaches you. If a request to act still reaches you here, it was not recognized as a task: do NOT pretend you did it and do NOT deny you can — tell the user you can do it and ask them to rephrase it as a direct instruction (e.g. "list the files in <folder>").
- TASK OUTCOME HONESTY: task and background-task outcome messages in this conversation are the COMPLETE record of what was done and found. Never add, infer, or embellish results (file names, counts, contents) beyond what those messages literally state. If an outcome message does not contain the answer the user wants, say the task did not report it and offer to run it again — never fill the gap yourself.
- NEVER imitate system-generated messages ("Finished the background task…", "I've started working on that in the background…", "Reminder set — …", "Done — N step(s) completed.", approval prompts). Those texts are produced by the backend only, after real actions — writing them yourself is claiming actions that never happened.
- You cannot start, queue, or schedule any action from this conversation. Never say you "will" perform a computer action, that a task "has been initiated", or otherwise promise action — nothing you write here makes anything happen. When the user wants an action, the ONLY honest reply is to ask them to say it as one direct instruction (e.g. "delete the .txt files in the phase3test folder on my desktop").
